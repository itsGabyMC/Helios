...
try {
    http.get('http://launcher.superkooka.com/obsidia/money/?uuid='+ require('./assets/js/configmanager').getSelectedAccount().uuid, (resp) => {
        let data = ''
        resp.on('data', (chunk) => {
            data += chunk
        })
        resp.on('end', () => {
            console.log(data)
            document.getElementById('moneyValue').textContent = data
        })
    }).on('error', (err) => {
...
const refreshServerStatus = async function(fade = false){
    ...
    try {
        ...
        loggerLanding.warn('Impossible d\'actualiser l\'état du serveur, supposant qu\'il est hors ligne..')
        loggerLanding.debug(err)
    }
    if(fade){
        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    ...
                    'Pour rejoindre Obsidia, vous avez besoin d\'une installation 64 bits de Java 8. Souhaitez-vous que nous installions une copie? En installant, vous acceptez <a href="http://www.oracle.com/technetwork/java/javase/terms/license/index.html">le contrat de licence d\'Oracle</a>.',
                    'Installer Java',
                ...
                setDismissHandler(() => {
                    $('#overlayContent').fadeOut(250, () => {
                        ...
                        setOverlayContent(
                            ...
                            'Une installation x64 valide de Java 8 est requise pour démarrer.<br><br>Merci de vous referez à notre <a href="https://github.com/dscalzi/HeliosLauncher/wiki/Java-Management#manually-installing-a-valid-version-of-java">guide Java</a> pour savoir comment installer manuellement Java.',
                            'Je comprends',
                            'Retourner'
                        )
                        setOverlayHandler(() => {
...
function dlAsync(login = true){
    ...
    aEx.on('error', (err) => {
        ...
        showLaunchFailure('Erreur lors du lancement', err.message)
    })
    aEx.on('close', (code, signal) => {
        if(code !== 0){
            loggerLaunchSuite.error(`AssetExec s'est terminer avec le code ${code}, en supposant une erreur.`)
        ...
        if(m.context === 'validate'){
            switch(m.data){
                case 'distribution':
                    ...
                    loggerLaunchSuite.log('distribution.json validé')
                ...
                case 'assets':
                    ...
                    loggerLaunchSuite.log('Validation des assets términer')
                    setLaunchDetails('Validation de l\'intégrité des librairies..')
                    break
                case 'libraries':
                    setLaunchPercentage(80, 100)
                    loggerLaunchSuite.log('Validation des libraries complète')
                ...
                case 'download':
                    ...
                    if(m.error.code === 'ENOENT'){
                        showLaunchFailure(
                            ...
                            'Impossible de se connecter au serveur des fichiers. Assurez-vous que vous êtes connecté à Internet et réessayez.'
                        )
                    } else {
                        showLaunchFailure(
                            'Erreur de téléchargement',
            ...
            if(m.result.forgeData == null || m.result.versionData == null){
                ...
                showLaunchFailure('Erreur lors du lancement')
            ...
            if(login && allGood) {
                ...
                const gameStateChange = function(data){
                    ...
                    if(SERVER_JOINED_REGEX.test(data)){
                        DiscordWrapper.updateDetails('Joue sur Obsidia')
                ...
                const gameErrorListener = function(data){
                    ...
                    if(data.indexOf('Impossible de trouver ou de charger la classe principale net.minecraft.launchwrapper.Launch') > -1){
                        ...
                        showLaunchFailure('Erreur lors du lancement', 'TLe fichier principal, LaunchWrapper, n\'a pas pu être téléchargé correctement. Par conséquent, le jeu ne peut pas se lancer.<br><br> Pour résoudre ce problème, désactivez temporairement votre logiciel antivirus et relancez le jeu. <br><br> Si vous avez le temps, veuillez <a href ="https://github.com/dscalzi/HeliosLauncher/issues"> soumettez un problème</a> et indiquez-nous quel logiciel antivirus vous utilisez. Nous les contacterons et essayerons de redresser les choses.')
                    }
                }
                ...
                try {
                    ...
                    showLaunchFailure('Erreur durant du lancement', 'Please check the console (CTRL + Shift + i) for more details.')
    ...
    refreshDistributionIndex(true, (data) => {
        ...
        refreshDistributionIndex(false, (data) => {
            ...
            if(DistroManager.getDistribution() == null){
                showLaunchFailure('Erreur fatale', 'Could not load a copy of the distribution index. See the console (CTRL + Shift + i) for more details.')
...
function setNewsLoading(val){
    if(val){
        const nLStr = 'Rafraîchissement des actualitées'
        let dotStr = '..'
