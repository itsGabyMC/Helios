        ...
        val.onclick = (e) => {
            ...
            for(let i=0; i<selectBtns.length; i++){
                if(selectBtns[i].hasAttribute('selected')){
                    ...
                    selectBtns[i].innerHTML = 'Compte séléctionné'
                }
            }
    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                isLastAccount = true
                setOverlayContent(
                    'Attention<br>C\'est votre dernier compte connecter',
        ...
        authAccountStr += `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            ...
            <div class="settingsAuthAccountRight">
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>Compte séléctionné &#10004;' : '>Selectionner ce compte'}</button>
                    <div class="settingsAuthAccountWrapper">
                        <button class="settingsAuthAccountLogOut">Déconnxion</button>
                    </div>  
                </div>
            </div>
        </div>`
    ...
    jg._validateJavaBinary(execPath).then(v => {
        if(v.valid){
            if(v.version.major < 9) {
                settingsJavaExecDetails.innerHTML = `Choisi: Java ${v.version.major} Mise à jour ${v.version.update} (x${v.arch})`
            } else {
...
function populateReleaseNotes(){
    $.ajax({
        url: 'https://github.com/Superkooka/Obsidia-Launcher/releases.atom',
        success: (data) => {
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        if(process.platform === 'darwin'){
            settingsUpdateButtonStatus('Download from GitHub<span style="font-size: 10px;color: gray;text-shadow: none !important;">Fermez le lanceur et exécutez le dmg pour mettre à jour.</span>', false, () => {
