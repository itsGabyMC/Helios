    ...
    Array.from(document.getElementsByClassName('settingsAuthAccountLogOut')).map((val) => {
        val.onclick = (e) => {
            ...
            if(Object.keys(ConfigManager.getAuthAccounts()).length === 1){
                isLastAccount = true
                setOverlayContent(
                    'Attention<br>Ceci est ton dernier compte conneté',
                    'Il faudras se reconnecter pour utiliser nos services.',
                    'Ce déconnecter',
                    'Annuler'
                )
                setOverlayHandler(() => {
        ...
        authAccountStr += `<div class="settingsAuthAccount" uuid="${acc.uuid}">
            ...
            <div class="settingsAuthAccountRight">
                ...
                <div class="settingsAuthAccountActions">
                    <button class="settingsAuthAccountSelect" ${selectedUUID === acc.uuid ? 'selected>Compte selectionnée &#10004;' : '>Choisir ce compte'}</button>
                    <div class="settingsAuthAccountWrapper">
                        <button class="settingsAuthAccountLogOut">Quitter</button>
                    </div>
                </div>
            </div>
        </div>`
