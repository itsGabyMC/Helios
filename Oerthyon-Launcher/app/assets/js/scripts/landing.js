...
function updateSelectedServer(serv){
    ...
    ConfigManager.save()
    server_selection_button.innerHTML = '\u2022 ' + (serv != null ? serv.getName() : 'Aucune version séléctionée')
    if(getCurrentView() === VIEWS.settings){
        ...
        if(m.context === 'validateJava'){
            if(m.result == null){
                ...
                setOverlayContent(
                    ...
                    'Pour rejoindre Oerthyon, vous avez besoin d\'une installation 64 bits de Java 8. Souhaitez-vous que nous installions une copie? En installant, vous acceptez <a href="http://www.oracle.com/technetwork/java/javase/terms/license/index.html">le Contrat de licence d\'Oracle</a>.',
                    'Install Java',
                    'Install Manually'
                )
                setOverlayHandler(() => {
...
function dlAsync(login = true){
        ...
        if(m.context === 'validate'){
            switch(m.data){
                case 'distribution':
                    ...
                    loggerLaunchSuite.log('Fichier de distribution validé.')
                ...
                case 'assets':
                    ...
                    setLaunchDetails('Validation de l\'intégrité de des libs ..')
                    break
                case 'libraries':
                    setLaunchPercentage(80, 100)
                    loggerLaunchSuite.log('Validation de la lib terminée.')
            ...
            if(login && allGood) {
                ...
                const gameStateChange = function(data){
                    ...
                    if(SERVER_JOINED_REGEX.test(data)){
                        ...
                        DiscordWrapper.updateDetails('Viens me rejoindre sur Oerthyon')
                    }
                }
                ...
                try {
                    ...
                    if(distro.discord != null && serv.discord != null){
                        ...
                        proc.on('close', (code, signal) => {
                            loggerLaunchSuite.log('Coupure de Discord Rich Presence..')
    ...
    refreshDistributionIndex(true, (data) => {
        ...
        loggerLaunchSuite.log('Erreur lors de la récupération d\'une nouvelle copie du fichier de distribution.', err)
        refreshDistributionIndex(false, (data) => {
            onDistroRefresh(data)
            serv = data.getServer(ConfigManager.getSelectedServer())
            aEx.send({task: 'execute', function: 'validateEverything', argsArr: [ConfigManager.getSelectedServer(), DistroManager.isDevMode()]})
        }, (err) => {
            loggerLaunchSuite.error('Impossible de rafraîchir le fichier de distribution.', err)
            if(DistroManager.getDistribution() == null){
                showLaunchFailure('Erreur fatale', 'Impossible de charger une copie du fichier de distribution.')
